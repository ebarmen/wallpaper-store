...
export function TextureElement(props: ITextureElementProps) {
  ...
  const setNewBackgroundData = () => {
    if (removeElements && defaultBackgroundData) {
      const newBackgroundDataItems = defaultBackgroundData.items.filter(
        (backgroundGroup) => {
      ...
      setBackgroundData({
        img: isChangeBackground
          ? newBackgroundDataItems[0].colors[0].img
          : backgroundData.img,
        name: isChangeBackground
          ? newBackgroundDataItems[0].colors[0].name
          : backgroundData.name,
        price: isChangeBackground
          ? newBackgroundDataItems[0].price
          : backgroundData.price,
        groupName: isChangeBackground
          ? newBackgroundDataItems[0].groupName
  ...
  return (
    ...
    >
      ...
      <div
        id={
          selected && !onHandleCategoryChange && !onHandleCollectionChange
            ? "element_name"
            : selected && onHandleCategoryChange
            ? "element_category"
            : ""
        }
      ...
      {price && !onHandleCategoryChange && !onHandleCollectionChange && (
        <div>
          <div className={classNames(styles.text, "price_font")}>
            {price} р./м2
          </div>
          <div
            className={`${styles.text}${
              selected ? " js-selected-texture-name" : ""
            }`}
          >
            ID {id}
          </div>
