...
function App() {
  ...
  const [newBackgroundData, setNewBackgroundData] = useState(
  ...
  const defaultIsSeamless =
    url.searchParams.get("isSeamless") === "true" ?? false;
  ...
  const [mode, setNewMode]: [string, Dispatch<SetStateAction<any>>] = useState(
    defaultMode ? defaultMode : ModeNames.Handmode,
  );
  const [isSeamless, setIsSeamless]: [boolean, Dispatch<SetStateAction<any>>] =
    useState(defaultIsSeamless);
  const [isSigned, setIsSigned]: [boolean, Dispatch<SetStateAction<any>>] =
    useState(defaultIsSigned);
  const [defaultCategoryIndex, setDefaultCategoryIndex] = useState<number>(0);
  const [defaultCollectionIndex, setDefaultCollectionIndex] =
    useState<number>(0);
  ...
  useEffect(() => {
    if (getConverter) setIsConverter(true);
    else setIsConverter(false);
  }, []);
  ...
  useEffect(() => {
    ...
    switch (initialImgData.catalog) {
      case CategoryNames.Classic:
        setNewBackgroundData({
          ...initialBackgroundData,
          items: initialBackgroundData.items.filter(
            (item) => !defaultExcludedBackgrounds.classic.includes(+item.id),
          ),
        });
        break;
      case CategoryNames.Geo:
        setNewBackgroundData({
          ...initialBackgroundData,
          items: initialBackgroundData.items.filter(
            (item) => !defaultExcludedBackgrounds.geo.includes(+item.id),
          ),
        });
        break;
      case CategoryNames.Pr:
        setNewBackgroundData({
          ...initialBackgroundData,
          items: initialBackgroundData.items.filter(
            (item) => !defaultExcludedBackgrounds.pr.includes(+item.id),
          ),
        });
        break;
    }
    ...
    for (let i = 0; i < Object.keys(initialImgData.fullData).length; i++) {
      if (
        (Object.values(initialImgData.fullData)[i] as any).name ===
        defaultCategory
      ) {
        ...
        for (
          ...
          j <
          Object.keys(
            (Object.values(initialImgData.fullData)[i] as any).collection,
          ).length;
          j++
        ) {
          if (
  ...
  return (
    <CanvasTotalPriceProvider
      value={{ price: totalPrice, setTotalPrice: setNewTotalPrice }}
    >
      <CanvasModeProvider
        value={{
          mode: mode,
          ...
          setIsSeamless: setIsSeamless,
          isSigned: isSigned,
          setIsSigned: setIsSigned,
        }}
      >
        <CanvasOptionsProvider
          value={{ options: newOptionsData, setOptions: setNewOptionsData }}
        >
          <CanvasHandFrameSizeProvider
            value={{
              handFrameData: handFrameData,
              setHandFrameData: setNewHandFrameData,
            }}
          >
            <CanvasImgProvider
              value={{
                imgData: {
                  img: newImgData.img,
                  price: newImgData.price,
                  catalog: newImgData.catalog,
                  items: newImgData.items,
                  fullData: newImgData.fullData,
                },
                setImgData: setNewImgData,
              }}
            >
              <CanvasBackgroundProvider
                value={{
                  backgroundData: {
                    img: newBackgroundData.img,
                    name: newBackgroundData.name,
                    price: newBackgroundData.price,
                    groupName: newBackgroundData.groupName,
                    items: newBackgroundData.items,
                  },
                  setBackgroundData: setNewBackgroundData,
                }}
              >
                {initialImgData && !isConverter && (
                  <div className="App">
                    <TextureSelect
                      defaultBackgroundData={initialBackgroundData}
                      defaultCategoryIndex={defaultCategoryIndex}
                      defaultCollectionIndex={defaultCollectionIndex}
                    />
                    <Constructor
                      mode={mode}
                      isSeamless={isSeamless}
