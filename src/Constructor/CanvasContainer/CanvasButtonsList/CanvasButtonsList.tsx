...
export function CanvasButtonsList(props: ICanvasButtonsList) {
    ...
    const makeWebLink = () => {
        const selectedOptions = document.getElementsByClassName('js-selected-option');
        ...
        options?.items.forEach(item => {
            if (item.name === OptionNames.Example && item.isExtraPrice) isExtraPrice = true;
        });

        const
            name = document.getElementById('element_name')?.innerHTML,
            ...
            params =
                ...
                '&gridCnt=' + gridCnt +
                '&dragX=' + dragX +
                '&handFrame=' + JSON.stringify(handFrameData) +
                '&seamless=' + seamless +
                '&standartSizes=' + standartSizes +
                '&signature=' + signature +
                '&varnish=' + varnish +
                '&install=' + install +
                '&example=' + example +
        ...
        const weblink = window.location.protocol + '//' + window.location.host + window.location.pathname + '?' + params;
    ...
    const handleMakeWebLink = () => {
        ...
        navigator.clipboard.writeText(weblink).then(function () {
    ...
    const handleDownloadPDF = () => {
        if (options) {
            ...
            const input = document.getElementsByClassName('js-canvas-pdf')[0],
                ...
                curDate = dd + '.' + mm + '.' + yyyy + ' ' + dateObj.getHours() + ':' + dateObj.getMinutes(),
                warningText = '*ВНИМАНИЕ! Без подписи заказ в производство не запускается!\n' +
                ...
                infoData =  `Название: ${imgData.items[0].name}\n` +
                            `Фон: ${backgroundData.name}\n` +
                            `Размеры: ${Math.round(handFrameData.width)} см на ${Math.round(handFrameData.height)} см\n` +
                            `Площадь: ${Math.round((handFrameData.width * handFrameData.height)/100)/100} м2\n` +
                            `Цена: ${price} р.`,
                optionsData = options.items[6].isHidden 
                        ? 'ОПЦИИ:\n' +
                            `${OptionNames.Seamless}: ${options.items[0].isHidden ? '-' : options.items[0].isSet ? 'да' : 'нет'}\n` +
                            `${OptionNames.StandartSizes}: ${options.items[1].isHidden ? '-' : options.items[1].isSet ? 'да' : 'нет'}\n` +
                            `${OptionNames.Signature}: ${options.items[2].isHidden ? '-' : options.items[2].isSet ? 'да' : 'нет'}\n` +
                            `${OptionNames.Varnish}: ${options.items[3].isHidden ? '-' : options.items[3].isSet ? 'да' : 'нет'}\n` +
                            `${OptionNames.Install}: ${options.items[4].isHidden ? '-' : options.items[4].isSet ? 'да' : 'нет'}\n` +
                            `${OptionNames.Example}: ${options.items[5].isHidden ? '-' : options.items[5].isSet ? 'да' : 'нет'}`
                        : 'ОПЦИИ:\n' +
                            `${OptionNames.Seamless}: ${options.items[0].isHidden ? '-' : options.items[0].isSet ? 'да' : 'нет'}\n` +
                            `${OptionNames.StandartSizes}: ${options.items[1].isHidden ? '-' : options.items[1].isSet ? 'да' : 'нет'}\n` +
                            `${OptionNames.Signature}: ${options.items[2].isHidden ? '-' : options.items[2].isSet ? 'да' : 'нет'}\n` +
                            `${OptionNames.Varnish}: ${options.items[3].isHidden ? '-' : options.items[3].isSet ? 'да' : 'нет'}\n` +
                            `${OptionNames.Install}: ${options.items[4].isHidden ? '-' : options.items[4].isSet ? 'да' : 'нет'}\n` +
                            `${OptionNames.Examples}: ${options.items[6].isHidden ? '-' : options.items[6].isSet ? 'да' : 'нет'}`
            ;
