...
export function CanvasContainer(props: ICanvasContainerProps) {
    ...
    const handlePlusClick = (side: string) => {
        const newGridCnt = gridCnt + 1 > maxGridCnt
            ? gridCnt
            : gridCnt + 1
        ;
    ...
    const handleMinusClick = (side: string) => {
        const newGridCnt = gridCnt - 1 < minGridCnt
            ? gridCnt
            : gridCnt - 1
        ;
        setGridCnt(newGridCnt);
        ...
        <div id={'canvas-container'} className={styles.container}>
            {canvasWidth && (
                <Canvas
                    ...
                    dragX={dragX}
                    onDragXChange={(newDragX) => {setDragX(newDragX)}}
                />
            )}
            {mode === ModeNames.Panels && (
                <div className={`${styles.plusMinusContainer} ${styles.left}`}>
                    <div
                        className={`${styles.iconContainer} ${gridCnt < 12 ? '' : styles.disabled}`}
                    ...
                    <div
                        className={`${styles.iconContainer} ${gridCnt > 7 ? '' : styles.disabled}`}
