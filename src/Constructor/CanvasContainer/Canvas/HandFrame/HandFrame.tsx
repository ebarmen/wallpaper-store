...
export const HandFrame = ({
    ...
    const handleMoveBackground = (e: any) => {
        const stage = e.target.getStage();
        const newPrevX = (mode === ModeNames.Handmode)
            ? prevX + (stage.getPointerPosition().x - startX)
            : Math.round((prevX + (stage.getPointerPosition().x - startX)) / (gridWidth * commonScaleX)) * (gridWidth * commonScaleX)
        ;
        if(!isMove) return;
    ...
    const handleMoveBackgroundEnd = (e: any) => {
        ...
        const newPrevX = ((prevX + (stage.getPointerPosition().x - startX)) < maxWidth * commonScaleX)
        ...
        <React.Fragment>
            <Group>
                ...
                <Rect
                    ...
                    onMouseDown={(mode === ModeNames.Panels) ? handleMoveBackgroundStart : () => {}}
                    onTouchStart={(mode === ModeNames.Panels) ? handleMoveBackgroundStart : () => {}}
                    onTouchMove={handleMoveBackground}
                    onMouseMove={handleMoveBackground}
                    onMouseUp={handleMoveBackgroundEnd}
                    onTouchEnd={handleMoveBackgroundEnd}
            ...
            <Transformer
                ...
                enabledAnchors={mode === ModeNames.Panels ? ['middle-right', 'middle-left'] : ['top-left', 'top-center', 'top-right', 'middle-right', 'middle-left', 'bottom-left', 'bottom-center', 'bottom-right']}
                ...
                boundBoxFunc={(oldBox, newBox) => {
                    if(Math.ceil(newBox.width) < Math.floor(gridWidth * commonScaleX) || newBox.height < 100) return oldBox;
                    ...
                    if(mode === ModeNames.Panels) {
                        ...
                        if ((Math.round(newBox.x / (gridWidth * commonScaleX)) * (gridWidth * commonScaleX) + indentX).toFixed(2) === oldBox.x.toFixed(2) && newBox.x < oldBox.x)
                            newBox.width = oldBox.width;
                        else if ((Math.round(newBox.x / (gridWidth * commonScaleX)) * (gridWidth * commonScaleX) + indentX).toFixed(2) > oldBox.x.toFixed(2))
                            newBox.width = Math.round((newBox.width - indentX) / (gridWidth * commonScaleX)) * (gridWidth * commonScaleX);
                        else
                            newBox.width = Math.round((newBox.width - indentX) / (gridWidth * commonScaleX)) * (gridWidth * commonScaleX);

                        newBox.x = Math.round(newBox.x / (gridWidth * commonScaleX)) * (gridWidth * commonScaleX) + indentX;
                    }
