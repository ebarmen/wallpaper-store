...
export function SingleOption(props: ISingleOptionProps) {
    ...
    const { mode, setMode, isSeamless, setIsSeamless, setIsSigned } = useContext(canvasModeContext);
    ...
    useEffect(() => {    
        ...
        if (options && name === OptionNames.Example) {
            setOptionPrice(Math.min(price + correction, priceExtra + extraCorrection));
    ...
    useEffect(() => {
        if (options && name === OptionNames.StandartSizes && mode === ModeNames.Handmode) {
            setIsChecked(true);
    ...
    return (
        <>
            <Tippy
                ...
                disabled={!isDisabled}
            >
                <div className={classNames(styles.container, 'row', isDisabled && styles.disabled)}>
                    <div className='row'>
                        <div className={styles.iconContainer} onClick={handleClick}>
                            <CheckboxIcon checked={isChecked}/>
                        </div>
                        <div className='row'>
                            <div className={classNames(styles.nameContainer, isChecked ? 'js-selected-option' : '', excluded ? 'js-hidden-option' : '')}>
                                {name}
                            </div>
                            <Tippy
                                className={styles.info}
                    ...
                    <div className={styles.priceContainer}>
                        {options && name === OptionNames.Example ? 'от ' : ''}{optionPrice} {isPricePerMeter ? 'р./м2' : 'р.'}
                    </div>
                </div>
            </Tippy>
            {options && name === OptionNames.Example && isChecked && 
                <div className={styles.extraOptions}>
                    {extraOptions.map((option, index) => <div key={index} className={`${styles.extraContainer} row`}>
                        <div className='row'>
                            <div className={classNames(styles.iconContainer, styles.iconContainerSmall)} onClick={() => handleExtraOptionClick(index, option.price)}>
                                <div className={classNames(styles.checkmark, checkedExtraOption === index && styles.checkmarkChecked)}/>
                            </div>
                            <div className={styles.nameContainer}>{option.name}</div>
                        </div>
                        <div className={styles.priceContainer}>
                            {option.price + option.correction} {isPricePerMeter ? 'р./м2' : 'р.'}
                        </div>
                    </div>)}
                </div>
            }
