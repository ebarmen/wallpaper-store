...
export function SizeSelector({ mode }: SizeSelectorProps) {
  ...
  const [wallHeight, setWallHeight] = useState(
    Math.round(handFrameData.height),
  );
  const [wallArea, setWallArea] = useState(
    ((handFrameData.height * handFrameData.width) / 10000).toFixed(2),
  );

  const setNewSizes = () => {
    const wallX = Math.round(
      handFrameData.x - (wallWidth - handFrameData.width) / 2,
    );
    const wallY = Math.round(
      handFrameData.y - (wallHeight - handFrameData.height) / 2,
    );
          ...
          <div className={classNames(styles.sizeInputs, "row")}>
            <div
              className={classNames(
                styles.sizeInput,
                mode === ModeNames.Panels && styles.standartOption,
                wallWidth > 1140 && styles.sizeInputError,
              )}
            >
              ...
              <input
                id={"width"}
                type="number"
                disabled={mode === ModeNames.Panels}
                value={wallWidth}
              ...
              {mode === ModeNames.Panels && (
                ...
                >
                  <path
                    d="M105.2,67.6c0-9.3,0-22.4-0.1-23c-1-14.1-12.9-25.1-27-25.1c-14.1,0-26,11-27,25.1c0,0.6-0.1,14.1-0.1,22.9
              ...
              {wallWidth > 665 && mode === ModeNames.Handmode && (
                <p className={styles.sizeError}>
                  Нестандартная ширина. Необходим индивидуальный раскрой
                  панелей.
                </p>
              )}
            </div>
            <div
              className={classNames(
                styles.sizeInput,
                mode === ModeNames.Panels && styles.standartOption,
                wallHeight > 290 && styles.sizeInputError,
              )}
            >
              ...
              {wallHeight > 290 && mode === ModeNames.Handmode && (
                <p className={styles.sizeError}>
                  Нестандартная высота. Необходим индивидуальный раскрой
                  панелей.
                </p>
              )}
            </div>
