...
export function ColorPicker(props: IColorPickerProps) {
  ...
  useEffect(() => {
    if (selected && imgData.catalog === CategoryNames.Pr)
      setBackgroundData({
        img: colors[0].img,
        name: colors[0].name,
        price: price,
          ...
          colors.map((color, index) => {
            return (
              <div
                key={index}
                onClick={() => {
                  handleClick(color.img, color.name);
                }}
      ...
      {premColors && premColors.length > 0 && (
        <div>
          ...
          <div className={styles.colorsList} onClick={handleSelect}>
            {premColors &&
              premColors.map((color, index) => {
                return (
                  <div
                    key={index}
                    onClick={() => handleClick(color.img, color.name, true)}
                    className={styles.colorItem}
                  >
                    <div
                      className={classNames(
