...
function calcTotalWidth(props: TCalcTotalWidthProps) {
  ...
  return (
    Math.round(
      (((Math.round(width) / 100) * Math.round(height)) / 100) * 10000,
    ) / 10000
  );
}
...
export function Total() {
  ...
  const calcOptionPrice = (
    ...
    options.forEach((item) => {
      if (item.isSet && !item.isPricePerMeter)
        optionsPrice += item.isExtraPrice
          ? item.priceExtra + item.priceExtraCorrection
          : item.price + item.priceCorrection;
      if (item.isSet && item.isPricePerMeter)
        optionsPrice +=
          (item.isExtraPrice
            ? item.priceExtra + item.priceExtraCorrection
            : item.price + item.priceCorrection) * totalWidth;
    });
    return optionsPrice;
  };
    ...
    <div>
      <div className={styles.container}>
        <div className={`${styles.list} row`}>
          ...
          <div id={"price"} data-price={price} className={styles.price}>
            <span>{price}</span> <span>р.</span>
            <span className={styles.star}> *</span>
          </div>
        </div>
        <div className={styles.bottomText}>
          <span className={styles.star}>* </span>Цена может измениться после
